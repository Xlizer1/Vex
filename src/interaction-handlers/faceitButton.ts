...
export class FaceitButtonHandler extends InteractionHandler {
  ...
  public override async run(interaction: ButtonInteraction) {
    const roleName = interaction.customId.slice(FACEIT_BUTTON_PREFIX.length);
    ...
    try {
      ...
      const result   = await RoleService.assignRole(interaction.member, interaction.guild, roleName, allNames);

      return interaction.reply({
        content: result === "added" ? `You now have the **${roleName}** role!` : `Removed the **${roleName}** role.`,
